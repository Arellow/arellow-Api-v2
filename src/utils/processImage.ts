...
// Validate image
function validateImage(file: ImageFile): void {
...
async function uploadWithRetry(file: string | { size: number; mimetype: string; content: string }): Promise<string> {
...
uploadQueue.process(async (job) => {
  const { image } = job.data as { image: ImageFile };
  try {
    validateImage(image);
