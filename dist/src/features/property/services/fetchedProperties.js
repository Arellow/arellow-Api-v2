...
class ProjectService {
    ...
    getFeaturedProjects() {
        return __awaiter(this, arguments, void 0, function* (page = 1, limit = 5, userId) {
            try {
                ...
                const featured = yield this.prisma.project.findMany({
                    where: whereClause,
                    take: limit,
                    skip,
                ...
                const totalCount = yield this.prisma.project.count({ where: whereClause });
                const data = yield Promise.all(featured.map((p) => __awaiter(this, void 0, void 0, function* () {
                    const isLiked = userId
                        ? (yield this.prisma.likeUser.findUnique({
                ...
                throw new appError_1.InternalServerError("Failed to fetch featured projects.");
            }
        });
    }
    getRecentProjects(filter, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const whereClause = {
                    ...
                    property_type: propertyType ? { equals: propertyType, mode: "insensitive" } : undefined,
                    number_of_bedrooms: bedrooms ? { equals: bedrooms } : undefined,
                    number_of_bathrooms: bathrooms ? { equals: bathrooms } : undefined,
                    archive: false,
                    status: "selling",
                    isapproved: "approved",
                };
                const recent = yield this.prisma.project.findMany({
                    where: whereClause,
                    take: limit,
                    skip,
                    orderBy: { createdAt: "desc" },
                ...
                const totalCount = yield this.prisma.project.count({ where: whereClause });
                const data = yield Promise.all(recent.map((p) => __awaiter(this, void 0, void 0, function* () {
                    const isLiked = userId
                        ? (yield this.prisma.likeUser.findUnique({
                            where: { userId_projectId: { userId, projectId: p.id } },
                        })) !== null
                ...
                throw new appError_1.InternalServerError("Failed to fetch recent projects.");
            }
        });
    }
    ...
    //     }
    //   }
    calculateMortgage(id, downPayment) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!project) {
                    throw new appError_1.InternalServerError("Project not found.");
                }
                ...
                if (isNaN(home_price) || isNaN(downPayment)) {
                    throw new appError_1.InternalServerError("'home_price' and 'down_payment' must be valid numbers.");
                }
                ...
                const principal_and_interest = parseFloat((loan_amount *
                    (monthly_interest * Math.pow(1 + monthly_interest, total_payments)) /
                    (Math.pow(1 + monthly_interest, total_payments) - 1)).toFixed(2));
                ...
                const total_monthly_payment = parseFloat((principal_and_interest +
                    monthly_property_tax +
                    monthly_home_insurance +
                    monthly_hoa +
                    monthly_mortgage_insurance).toFixed(2));
                const estimated_closing_cost = parseFloat((0.02 * home_price).toFixed(2));
                return {
                    home_location: project.property_location,
                    home_price,
                ...
                throw new appError_1.InternalServerError("Failed to calculate mortgage.");
            }
        });
    }
}
