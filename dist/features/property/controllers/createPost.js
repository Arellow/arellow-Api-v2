...
const trim_js_1 = require("../../../utils/trim.js");
...
const CreatePost = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        (0, trim_js_1.trimObjectKeys)(req.body);
    }
    catch (err) {
    ...
    try {
        parsed = {
            ...
            expected_pricing: expected_pricing
                ? toFloat(expected_pricing, "expected_pricing")
                : null,
            price_per_sqft: price_per_sqft
                ? toFloat(price_per_sqft, "price_per_sqft")
                : null,
            booking_amount: booking_amount
                ? toFloat(booking_amount, "booking_amount")
                : null,
            maintenance_monthly: maintenance_monthly
                ? toFloat(maintenance_monthly, "maintenance_monthly")
                : null,
        ...
        if (available_from &&
            ...
            isNaN(parsed.available_from.getTime())) {
            throw new appError_1.BadRequestError("available_from is not a valid date");
        }
    }
    catch (err) {
        console.error("Parsing numeric/date fields failed:", err);
        throw new appError_1.BadRequestError(`Invalid numeric/date field: ${err.message}. Check your inputs.`);
    }
    const { banner, outside_view_images, living_room_images, kitchen_room_images, primary_room_images, floor_plan_images, tour_3d_images, other_images, youTube_thumbnail, } = req.files || {};
    const processImages = (images, label) => __awaiter(void 0, void 0, void 0, function* () {
        if (!images)
            return [];
        try {
            return yield (0, processImage_1.processImage)(images);
        }
        catch (err) {
            console.error(`Processing ${label} failed:`, err);
            throw new appError_1.BadRequestError(`Failed to process ${label}. Make sure files are valid images.`);
        }
    });
    let postImages;
    try {
        ...
        throw new appError_1.BadRequestError(err.message);
    }
    ...
    try {
        ...
        if (err instanceof client_2.Prisma.PrismaClientValidationError) {
